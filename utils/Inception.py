...
class InceptionModelNew(nn.Module):
    ...
    def forward(self, x: torch.Tensor, get_ha=False, context=None) -> torch.Tensor:  # type: ignore
        ...
        if context==None:
            y = self.blocks(x).mean(dim=-1)  # the mean is the global average pooling
        else:
            y = context  # the mean is the global average pooling
        out=self.linear(y)
        if get_ha:
            return out, y,y,y,y#to compatible with rest of the code
        return out
...
class InceptionModel(nn.Module):
    ...
    def forward(self, x: torch.Tensor, get_ha=False) -> torch.Tensor:  # type: ignore
        ...
        if get_ha:
            return self.linear(x), x,x,x#to compatible with rest of the code
